...
head_to_class = {'cifar10': {0: 8, 1: 9, 2: 7},
                 'cifar100': {22: 99, 11: 98, 13: 89, 6: 88, 26: 78, 4: 77}}
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, log_training, tf_writer, h_to_tail)
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer, h_to_tail):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # print(target)
        for i in h_to_tail.keys():
            if random.random() < args.switch_prob:
                target[target == i] = torch.tensor(h_to_tail[i], dtype=torch.long)
        # print(target)
